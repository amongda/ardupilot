...
COMMAND_INT 필드 (COMMAND_LONG 아님): frame=1(LOCAL_NED, home 기준 NEU) /
param1=state(1~11) / param2=Target Heading(deg,진북) / x=North(cm) /
y=East(cm) / z=Altitude(m,up)  (x/y/z/param2 는 state 6 에서만 유효)

SITL용. 실기체 최종본은 frame=3(위경도) 을 쓰는 tdcn_gcs_Lat_Lon.py, 기체 코드는 동일.
...
-------
    ...
    ./tdcn_gcs.py --state 6 --north 12.5 --alt 20 --track 20   # 20초 스트리밍
...
# frame=1: 좌표가 home 기준 NEU (최종본은 3=MAV_FRAME_GLOBAL_RELATIVE_ALT)
...
@dataclass
class Target:
    """추종 비행 타겟. North/East/Alt는 m(Alt는 up-positive), Heading은 진북 기준 deg."""
    ...
    def as_command_int_fields(self) -> tuple[int, int, float, float]:
        """(x, y, z, heading) 로 변환 — x/y는 cm 단위 int32라 정밀도 손실이 없다."""
...
class TdcnGCS:
    def __init__(self, device: str, target_system: int = 0,
        ...
        # 기체가 마지막으로 받아들인 state (ACK 해석용, 기체는 NONE=0 에서 시작)
        self.state: int = 0
    ...
    def send(self, state: int, target: Target | None = None,
        ...
        # COMMAND_INT + frame=LOCAL_NED 로 전송 (최종본은 frame=3+위경도로 교체하면 됨)
