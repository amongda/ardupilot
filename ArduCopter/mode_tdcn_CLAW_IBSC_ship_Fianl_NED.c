...
/* [Sejong] : STV_OLD는 전역변수로 선언되어 있어, 한번만 초기화 되기 때문에 모드 재진입마다 초기화 하기 위한 플래그를 선언함*/
...
// [Sejong] : 아두파일럿 내에서 TDCN 모드 재진입마다 IBSC Home을 초기화하기 위해 static 제거
bool home_init = false;   
double Err_N, Err_E, Err_D;

// [Sejong] : IBSC 내부에서 계산되는 값들을 TDCN 모드로 가져와서 비교하기 위해 전역 변수로 변경
...
void CLAW_step(void)
{
    ...
    if (!home_init) {
        if (fabs(Cur_Lat) > 1.0 && fabs(Cur_Lon) > 1.0) {
            ...
            /* [Sejong] : x/y 스왑 수정 (이전에 언급하신 미션플래너에서 좌표가 반대되는 형상 때문에 하신 걸로 판단됨)
               원본:
            ...
            /* [Sejong] : 모드 재진입시 Home을 재초기화 하면서 STV_OLD도 초기화 하기 위한 플래그 설정 */
            stv_old_valid = false;
    ...
    if (home_init) {
        /* [Sejong]: Lon2m() 인자 수정 (저희가 판단하기에 인자가 잘못되었다고 판단되어, 한번 더 확인해주시면 감사하겠습니다)
    ...
    /* [Sejong] : TDCN 모드는 CLAW_step()를 호출하는 방식으로 사용하기 때문에 호출 전에 XTV[0~2]에 직접 넣어주기 때문에 해당 부분을 주석처리 해야 EKF 속도값이 보장됨
    XTV[0] = 0;
    XTV[1] = 0;
    XTV[2] = 0;                                                             */
...
#define ACCEL_LIMIT_ANGULAR 10.0
    ...
    /* [Sejong] : CTML_inv -> CTML 로 수정(이 부분도 한번 확인해주시면 감사하겠습니다)
       원본:
    ...
    /* Sejong: 롤 부호 반전 (이 부분은 저희가 아두파일럿 PID제어와 비교했을 때 롤만 부호가 반대되어 임시로 수정했습니다. 위와 같은 Lon2m() 인자 때문에 +로 두신 게 아닌가 생각됩니다)
       원본:
    ...
    /* [Sejong] : 위와 같은 이유로 부호 반전 (로그를 다시 확인해보니 피치도 부호를 바꿔야할 것 같습니다 확인부탁드리겠습니다)
    ...
    /* [Sejong] : TDCN으로 가져와서 분석하기 위해 캐스팅을 제거
       원본:
